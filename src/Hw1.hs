...
digitsOfInt num
  | num <= 0    = []
  | otherwise = digitsOfInt (num `div` 10) ++ [num `mod` 10]
...
additivePersistence num
  | num < 10    = 0
  | otherwise = 1 + additivePersistence (sumList (digitsOfInt num))
...
digitalRoot num
  | num < 10    = num
  | otherwise = digitalRoot (sumList (digitsOfInt num))
...
listReverse :: [a] -> [a]
listReverse [] = []
listReverse (element:remainder) = listReverse remainder ++ [element]
